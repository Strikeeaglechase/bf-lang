...
class Scope {
    constructor(compiler, global, parent, varIdx, arrIdx) {
        this.vars = [];
    ...
    getVar(varName) {
        ...
        if (local)
            return local;
        if (this.global)
            return this.global.vars.find(v => v.name == varName);
...
function count(target, propertyKey, descriptor) {
    ...
    descriptor.value = function (...args) {
        if (!this.counter[func.name])
            this.counter[func.name] = 0;
...
class Compiler {
    constructor(prog) {
        ...
        this.funcs = [].concat(builtinFunctions);
        // -- Debug -- 
        this.idxs = [];
        ...
        this.stack = [new Frame(this.globalScope, 0)];
    ...
    compileNode(tree) {
        switch (tree.type) {
            ...
            case AST.NodeType.while:
                this.handleWhile(tree);
                break;
            case AST.NodeType.if:
                this.handleIf(tree);
                break;
            case AST.NodeType.typeDef:
                this.handleTypeDef(tree);
                break;
        }
    }
    handleWhile(whileDef) {
    ...
    handleIf(ifDef) {
        ...
        this.code += `[`;
        for (let i = 0; i < ifDef.inside.length; i++)
            this.compileNode(ifDef.inside[i]);
        this.goto(MATH_TEMP_STORE);
        this.code += `[-]]`;
    }
    return(ret) {
...
__decorate([
    count
], Compiler.prototype, "fetchFromArray", null);
__decorate([
    count
], Compiler.prototype, "setArrayValue", null);
__decorate([
    count
], Compiler.prototype, "copy", null);
__decorate([
    count
], Compiler.prototype, "makeValue", null);
__decorate([
    count
], Compiler.prototype, "move", null);
export { Compiler };
